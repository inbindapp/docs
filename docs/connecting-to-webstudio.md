...
4. Provide your S3 or Cloudflare R2 credentials (see [storage setup](#storage-setup) below)
...
3. In the Variables panel, click **+** and select **Resource**
4. Name it (e.g., `All Posts`)
...
7. Bind its **Data** property to the items array from your Resource (e.g., `All_Posts.items`)
...
2. Select the **Body** element in the Navigator panel
3. Add a **Resource** variable (e.g., `Post Data`)
...
5. Add components to the page and bind them to the Resource data:
   - **Heading** — Bind text content to `Post_Data.title`
   - **Content Embed** — Bind to `Post_Data.body` (for HTML rich text content)
   - **Image** — Bind src to `Post_Data.image` and alt text accordingly
...
2. Bind **Meta Title** to `Post_Data.title`
3. Bind **Meta Description** to `Post_Data.description` (or another suitable field)
4. Bind **Social Image** to `Post_Data.image`

### Handling 404 Pages

When someone visits a URL with a slug that doesn't exist, the Resource will return empty data. To handle this:

1. In Page Settings, set the **Status Code** to an expression: `!Post_Data.title ? 404 : 200`
2. Create two sections on the page — one for content, one for a "Page not found" message
3. Set the **Show** property of the content section to `Post_Data.title` (shows when data exists)
4. Set the **Show** property of the 404 section to `!Post_Data.title` (shows when no data)
...
When creating a Webstudio connection, you need to provide S3-compatible storage credentials:
...
| Public URL | Your R2 public bucket URL (e.g., `https://pub-xxx.r2.dev` or custom domain) |
:::

The **Public URL** is important — it determines the base URL for your content that Webstudio will use to fetch data.
